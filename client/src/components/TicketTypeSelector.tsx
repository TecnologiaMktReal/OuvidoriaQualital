...
export function TicketTypeSelector({ ticket }: { ticket: SimpleTicket }) {
  const [open, setOpen] = useState(false);
  const [search, setSearch] = useState("");
  
  const ctx = trpc.useContext();
  const { data: types } = trpc.ticketSetup.ticketTypes.list.useQuery();
  ...
  const currentType = types?.find(t => t.id === ticket.ticketTypeId) || (ticket.ticketTypeId ? null : defaultType);

  // Use current type color or default or fallback
  const bgColor = currentType?.color || "#f1f5f9"; // slate-100
  const textColor = currentType?.color ? "#fff" : "#64748b"; // slate-500
  const borderColor = currentType?.color ? "transparent" : "#e2e8f0"; // slate-200

  const handleSelect = (ticketTypeId: number | undefined) => {
    updateDetails.mutate({ id: ticket.id, ticketTypeId });
    setOpen(false);
    setSearch("");
  };
          ...
          <div className="max-h-[240px] overflow-y-auto space-y-1 scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent pr-1">
            ...
            {filteredTypes?.map((type) => (
              ...
              >
                ...
                {type.isDefault && <span className="text-[10px] text-slate-400 bg-slate-100 px-1 rounded">Default</span>}
              </button>
            ))}
